...
if __name__ == "__main__":
    ...
    data_config = parse_data_config(opt.data_config)#读取数据配置
    valid_path = data_config["valid"]#验证集
    class_names = load_classes(data_config["names"])#类别名称

    # Initiate model
    model = Darknet(opt.model_def).to(device)#初始化模型
    if opt.weights_path.endswith(".weights"):#导入权重参数
    ...
    precision, recall, AP, f1, ap_class = evaluate(
        ...
        img_size=opt.img_size, #3×416×416
